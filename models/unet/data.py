...
# path to images dataset folder (that contains the 'train', 'test', and 'validate' folders)
data_path = '../../output/combined'
image_rows = 384
image_cols = 384
...
def create_train_data():
    ...
    for image_name in images:
        ...
        # Temp: Don't continue if no mask image
        if (os.path.exists( os.path.join(train_data_path, image_mask_name) ) == False):
          continue
        ##
...
def create_validate_data():
    ...
    for image_name in images:
        ...
        image_mask_name = image_name.split('.')[0] + '_mask.png'
        # Temp: Don't continue if no mask image
        if (os.path.exists( os.path.join(validate_data_path, image_mask_name) ) == False):
          continue
        ##
...
def create_test_data():
    ...
    for image_name in images:
        ...
        image_mask_name = image_name.split('.')[0] + '_mask.png'
        # Temp: Don't continue if no mask image
        if (os.path.exists( os.path.join(test_data_path, image_mask_name) ) == False):
          continue
        ##
